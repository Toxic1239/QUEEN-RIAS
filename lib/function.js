...
const getTime = (format, date) => {
	if (date) {
		...
		return moment.tz('Africa/Lagos').locale('en-US').format(format)
	}
}

const formatDate = (n, locale = 'en-US') => {
	let d = new Date(n)
...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./riasmedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./riasmedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./riasmedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./riasmedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./riasmedia/trash/${filename}.mp4`), unlink(`./riasmedia/trash/${filename}.gif`)])
    return buffer5
}
